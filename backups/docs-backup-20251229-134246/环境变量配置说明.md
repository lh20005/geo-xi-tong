# 环境变量配置说明

## ✅ 最终方案：统一使用 server/.env

### 为什么只保留一个 .env 文件？

1. **避免混淆** - 两个 `.env` 文件容易导致配置不一致
2. **简化维护** - 只需要维护一个配置文件
3. **清晰明确** - 后端服务的配置就在后端目录

---

## 📁 文件结构

```
GEO系统/
├── .env                    ❌ 已删除
├── server/
│   ├── .env               ✅ 唯一的环境变量文件
│   └── src/
│       ├── index.ts       ✅ 读取 server/.env
│       └── db/
│           ├── database.ts ✅ 读取 server/.env
│           └── redis.ts    ✅ 读取 server/.env
```

---

## 🔧 代码修改

### 1. server/src/index.ts
```typescript
// 修改前
dotenv.config({ path: path.join(__dirname, '../../.env') });

// 修改后
dotenv.config({ path: path.join(__dirname, '../.env') });
```

### 2. server/src/db/database.ts
```typescript
// 修改前
dotenv.config({ path: path.join(__dirname, '../../../.env') });

// 修改后
dotenv.config({ path: path.join(__dirname, '../../.env') });
```

### 3. server/src/db/redis.ts
```typescript
// 修改前
dotenv.config({ path: path.join(__dirname, '../../../.env') });

// 修改后
dotenv.config({ path: path.join(__dirname, '../../.env') });
```

---

## 📝 server/.env 配置内容

```env
# 本地开发环境配置
NODE_ENV=development
PORT=3000

# 数据库配置
DATABASE_URL=postgresql://lzc@localhost:5432/geo_system
DB_HOST=localhost
DB_PORT=5432
DB_NAME=geo_system
DB_USER=lzc
DB_PASSWORD=

# JWT 配置
JWT_SECRET=01063967995933d396d9c8271612794d4c174665de8748d27c7abce6b366f46e
JWT_REFRESH_SECRET=e5c75b1c40c8457fff6955b99fab1e8eec8645cca0146981fa2fa7a1794f482a
JWT_EXPIRES_IN=1h
REFRESH_TOKEN_EXPIRES_IN=7d

# 管理员账号
ADMIN_USERNAME=lzc2005
ADMIN_PASSWORD=Woshixiaogou2005

# AI API 配置
DEEPSEEK_API_KEY=sk-your-deepseek-api-key
GEMINI_API_KEY=AIzaSy-your-gemini-api-key
OLLAMA_BASE_URL=http://localhost:11434

# WebSocket 配置
WEBSOCKET_PORT=8080

# CORS 配置
ALLOWED_ORIGINS=http://localhost,http://localhost:5173,http://localhost:5174,http://localhost:8080,https://granolithic-pseudoprosperous-rebeca.ngrok-free.dev

# 限流配置
LOGIN_RATE_LIMIT=5
LOGIN_RATE_WINDOW_MINUTES=15
REGISTRATION_RATE_LIMIT=3
REGISTRATION_RATE_WINDOW_HOURS=1

# ==================== 微信支付配置 ====================
WECHAT_PAY_APP_ID=wx76c24846b57dfaa9
WECHAT_PAY_MCH_ID=1103960104
WECHAT_PAY_API_V3_KEY=3453DGDsdf3gsd564DSFDSR2N67N8Lfs
WECHAT_PAY_SERIAL_NO=305B80592042FA4A46F7A68E10044169EE13093D
WECHAT_PAY_PRIVATE_KEY_PATH=/Users/lzc/.wechat-pay/apiclient_key.pem
WECHAT_PAY_NOTIFY_URL=https://granolithic-pseudoprosperous-rebeca.ngrok-free.dev/api/payment/wechat/notify
```

---

## ⚠️ 重要提示

### 更新 ngrok 地址时

当 ngrok 重启后域名改变时，需要更新两个地方：

1. **ALLOWED_ORIGINS** - 添加新的 ngrok 域名
2. **WECHAT_PAY_NOTIFY_URL** - 更新为新的 ngrok 域名

```env
# 示例
ALLOWED_ORIGINS=http://localhost,http://localhost:5173,http://localhost:5174,http://localhost:8080,https://新的ngrok域名.ngrok-free.dev
WECHAT_PAY_NOTIFY_URL=https://新的ngrok域名.ngrok-free.dev/api/payment/wechat/notify
```

### 密码说明

- **管理员密码**：`Woshixiaogou2005`（在 server/.env 中配置）
- 这是唯一有效的密码，之前的 `TestDocker2024!` 已废弃

---

## ✅ 验证配置

### 1. 检查服务启动
```bash
cd server
npm run dev
```

应该看到：
```
✅ 数据库连接成功
✅ Redis连接成功
✅ 支付配置验证通过
🚀 服务器运行在 http://localhost:3000
```

### 2. 测试登录
```bash
curl -X POST http://localhost:3000/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"username":"lzc2005","password":"Woshixiaogou2005"}'
```

应该返回：
```json
{
  "success": true,
  "data": {
    "token": "...",
    "user": {...}
  }
}
```

---

## 🎯 总结

✅ 删除了根目录的 `.env` 文件  
✅ 统一使用 `server/.env`  
✅ 修改了所有读取环境变量的代码  
✅ 服务器启动正常  
✅ 登录功能正常  
✅ CORS 配置正确  
✅ 微信支付配置正确  

**现在配置清晰明确，不会再混淆了！**
