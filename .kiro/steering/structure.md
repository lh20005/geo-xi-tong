...
├── server/                        # 后端 API（Node.js/Express）- 部署到服务器
├── landing/                       # 营销落地页 - 部署到服务器
├── windows-login-manager/         # Electron 桌面应用（完整前端 + 本地功能）⭐ 当前使用
...
**重要说明**:
- ⭐ **当前使用**: `windows-login-manager/` - Windows 桌面客户端（包含完整前端界面 + 本地功能）
- 🗄️ **已归档**: `client-archived-web-frontend/` - 原 Web 前端（已废弃，仅作备份，不要在此开发）
- 📌 **服务器**: 只部署后端 API 和落地页，不再部署 Web 前端

## Windows 桌面客户端结构 (windows-login-manager/)

**这是当前使用的前端应用**，包含完整的用户界面和本地功能：
...
## 归档的 Web 前端结构 (client-archived-web-frontend/)

**⚠️ 此目录已废弃，仅作备份参考，不要在此开发！**
