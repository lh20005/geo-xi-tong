...
const browserConfig_1 = require("../config/browserConfig");
const cookieNormalizer_1 = require("../utils/cookieNormalizer");
/**
 * 账号管理服务
 * 处理平台账号的CRUD操作
 */
class AccountService {
    ...
    async createAccount(input, userId) {
        ...
        // 加密凭证
        const encryptedCredentials = EncryptionService_1.encryptionService.encryptObject(input.credentials);
        const result = await database_1.pool.query(`INSERT INTO platform_accounts 
       (platform, platform_id, account_name, credentials, user_id, status, is_default) 
       VALUES ($1, $2, $3, $4, $5, $6, $7) 
       RETURNING *`, [input.platform_id, input.platform_id, input.account_name, encryptedCredentials, userId, 'active', false]);
     ...
     * 创建平台账号绑定（包含真实用户名）
     */
    async createAccountWithRealUsername(input, realUsername, userId) {
        // 验证凭证格式
        this.validateCredentials(input.credentials);
        // 加密凭证
        const encryptedCredentials = EncryptionService_1.encryptionService.encryptObject(input.credentials);
       ...
       RETURNING *`, [input.platform_id, input.platform_id, input.account_name, encryptedCredentials, realUsername, userId, 'active', false]);
        const account = result.rows[0];
        // 返回时不包含加密的凭证
        return this.formatAccount(account, false);
    }
     ...
     */
    async createOrUpdateAccount(input, realUsername, userId) {
        ...
        const existingResult = await database_1.pool.query(`SELECT id, platform_id, account_name, real_username, user_id FROM platform_accounts 
       WHERE platform_id = $1 
       AND user_id = $2
       AND (real_username = $3 OR (real_username IS NULL AND account_name = $3))
       LIMIT 1`, [input.platform_id, userId, uniqueIdentifier]);
        ...
        if (existingResult.rows.length > 0) {
            ...
            const updateResult = await database_1.pool.query(`UPDATE platform_accounts 
         SET credentials = $1, 
             real_username = $2,
             account_name = $3,
         ...
         RETURNING *`, [encryptedCredentials, realUsername, input.account_name, existingAccount.id, userId]);
            ...
            const encryptedCredentials = EncryptionService_1.encryptionService.encryptObject(input.credentials);
            console.log('[AccountService] 凭证加密完成，长度:', encryptedCredentials.length);
            const insertResult = await database_1.pool.query(`INSERT INTO platform_accounts 
         (platform, platform_id, account_name, credentials, real_username, user_id, status, is_default) 
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8) 
         RETURNING *`, [input.platform_id, input.platform_id, input.account_name, encryptedCredentials, realUsername, userId, 'active', false]);
    ...
    async getAllAccounts(userId) {
        const result = await database_1.pool.query(`SELECT * FROM platform_accounts 
       WHERE user_id = $1
     ...
     */
    async getAccountsByPlatform(platformId, userId) {
        const result = await database_1.pool.query(`SELECT * FROM platform_accounts 
       WHERE platform_id = $1 AND user_id = $2
       ORDER BY is_default DESC, created_at DESC`, [platformId, userId]);
        return result.rows.map(row => this.formatAccount(row, false));
    }
    /**
     * 根据ID获取账号（包含解密的凭证）- 验证所有权
     */
    async getAccountById(accountId, userId, includeCredentials = false) {
        const result = await database_1.pool.query('SELECT * FROM platform_accounts WHERE id = $1 AND user_id = $2', [accountId, userId]);
     ...
     * 更新账号
     */
    async updateAccount(accountId, input, userId) {
        const updates = [];
        const values = [];
        let paramIndex = 1;
        ...
        values.push(userId);
        const result = await database_1.pool.query(`UPDATE platform_accounts 
       SET ${updates.join(', ')} 
     ...
     * 更新账号（包含真实用户名）
     */
    async updateAccountWithRealUsername(accountId, input, realUsername, userId) {
        const updates = [];
        const values = [];
        let paramIndex = 1;
        if (input.account_name) {
    ...
    async deleteAccount(accountId, userId) {
        const result = await database_1.pool.query('DELETE FROM platform_accounts WHERE id = $1 AND user_id = $2', [accountId, userId]);
     ...
     * 设置默认账号 - 仅在当前用户的账号中设置
     */
    async setDefaultAccount(platformId, accountId, userId) {
        const client = await database_1.pool.connect();
        try {
            await client.query('BEGIN');
            // 取消该平台该用户所有账号的默认状态
            await client.query('UPDATE platform_accounts SET is_default = false WHERE platform_id = $1 AND user_id = $2', [platformId, userId]);
            // 设置指定账号为默认（验证所有权）
            const result = await client.query('UPDATE platform_accounts SET is_default = true WHERE id = $1 AND platform_id = $2 AND user_id = $3', [accountId, platformId, userId]);
            if (result.rowCount === 0) {
     ...
     * 获取平台的默认账号
     */
    async getDefaultAccount(platformId) {
        const result = await database_1.pool.query('SELECT * FROM platform_accounts WHERE platform_id = $1 AND is_default = true', [platformId]);
        if (result.rows.length === 0) {
            // 如果没有默认账号，返回第一个账号
            const firstResult = await database_1.pool.query('SELECT * FROM platform_accounts WHERE platform_id = $1 ORDER BY created_at ASC LIMIT 1', [platformId]);
    ...
    async updateLastUsed(accountId) {
        await database_1.pool.query('UPDATE platform_accounts SET last_used_at = CURRENT_TIMESTAMP WHERE id = $1', [accountId]);
    }
    /**
     * 更新账号状态
     ...
     */
    async updateAccountStatus(accountId, status, userId) {
        console.log(`[AccountService] 更新账号状态: ID=${accountId}, status=${status}`);
        if (userId) {
            // 验证所有权
            const result = await database_1.pool.query('UPDATE platform_accounts SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 AND user_id = $3', [status, accountId, userId]);
            if (result.rowCount === 0) {
                throw new Error('账号不存在或无权访问');
            }
        }
            ...
            // 不验证所有权（系统内部调用）
            await database_1.pool.query('UPDATE platform_accounts SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [status, accountId]);
        }
     ...
     * @param reason 掉线原因
     */
    async markAccountOffline(accountId, reason = 'Cookie已失效') {
        console.log(`[AccountService] 标记账号为掉线: ID=${accountId}, reason=${reason}`);
        await database_1.pool.query(`UPDATE platform_accounts 
       SET status = 'expired', 
           updated_at = CURRENT_TIMESTAMP 
     ...
     * @param accountId 账号ID
     */
    async markAccountOnline(accountId) {
    ...
    async loginWithBrowser(platform, userId) {
        ...
        try {
            ...
            console.log(`========================================\n`);
            // 查找系统Chrome路径
            const executablePath = (0, browserConfig_1.findChromeExecutable)();
            // 使用 Playwright 启动浏览器
            ...
            if (navConfig) {
                ...
                if (platform.platform_id === 'jianshu' && navConfig.needsUserPage) {
     ...
     * 获取平台登录URL
     */
    getPlatformLoginUrl(platformId) {
        const loginUrls = {
            // 主流自媒体平台
     ...
     * 根据不同平台使用不同的检测策略
     */
    async waitForLogin(page, platformId) {
        ...
        const loginDetectionStrategies = {
            ...
            'souhu': async () => {
             ...
             (window.location.href.includes('/main/') || 
              ...
              window.location.href.includes('/page'))`, { timeout: 300000 });
                    const currentUrl = page.url();
            ...
            'xiaohongshu': async () => {
                ...
                try {
                    // 等待URL跳转到新版主页（精准判断）
                    await page.waitForFunction(`window.location.href.includes('creator.xiaohongshu.com/new/home')`, { timeout: 300000 });
            ...
            'wechat': async () => {
                ...
                try {
                    ...
                    await page.waitForFunction(`window.location.href.includes('mp.weixin.qq.com/cgi-bin/') && 
             window.location.href.includes('token=')`, { timeout: 300000 });
                    const currentUrl = page.url();
            ...
            'bilibili': async () => {
                ...
                try {
                    ...
                    await page.waitForFunction(`window.location.href.includes('member.bilibili.com/platform')`, { timeout: 300000 });
                    const currentUrl = page.url();
            ...
            'wangyi': async () => {
                ...
                try {
                    ...
                    await page.waitForFunction(`!window.location.href.includes('login') && 
             window.location.href.includes('mp.163.com')`, { timeout: 300000 });
                    const currentUrl = page.url();
            ...
            'baijiahao': async () => {
             ...
             window.location.href.includes('baijiahao.baidu.com')`, { timeout: 300000 });
                    const currentUrl = page.url();
            ...
            'csdn': async () => {
                ...
                try {
                    ...
                    await page.waitForFunction(`!window.location.href.includes('passport') && 
             !window.location.href.includes('login') &&
             (window.location.href.includes('csdn.net') || window.location.href.includes('blog.csdn.net'))`, { timeout: 300000 });
                    const currentUrl = page.url();
            ...
            'zhihu': async () => {
             ...
             !window.location.href.includes('login') &&
             window.location.href.includes('zhihu.com')`, { timeout: 300000 });
                    const currentUrl = page.url();
            ...
            'qie': async () => {
                ...
                try {
                    ...
                    await page.waitForFunction(`!window.location.href.includes('userAuth') && 
             ...
             (window.location.href.includes('/main') ||
              ...
              window.location.href.includes('/content'))`, { timeout: 300000 });
                    const currentUrl = page.url();
            ...
            'jianshu': async () => {
                ...
                try {
                    ...
                    await page.waitForFunction(`window.location.href === 'https://www.jianshu.com/' || 
             (window.location.href.includes('jianshu.com') && 
              !window.location.href.includes('sign_in') && 
              !window.location.href.includes('sign_up'))`, { timeout: 60000 });
                }
                const currentUrl = page.url();
     ...
     */
    async extractUserInfo(page, platformId) {
        try {
            console.log(`\n========================================`);
            ...
            for (let i = 0; i < selectorList.length; i++) {
                ...
                try {
                    ...
                    if (element) {
                        ...
                        username = await page.$eval(selector, (el) => el.textContent?.trim() || '');
                        if (username) {
     ...
     */
    async extractUserInfoViaAPI(page, platformId) {
        try {
     ...
     * 格式化账号数据
     */
    formatAccount(row, includeCredentials) {
        const account = {
            id: row.id,
        ...
        if (!account.real_username && row.credentials) {
            try {
                const decryptedCredentials = EncryptionService_1.encryptionService.decryptObject(row.credentials);
     ...
     */
    async testAccountLogin(accountId, userId) {
        try {
            const { chromium } = require('playwright');
            // 获取账号信息（包含凭证）
            ...
            // 获取平台信息
            const platformResult = await database_1.pool.query('SELECT * FROM platforms_config WHERE platform_id = $1', [account.platform_id]);
            ...
            const normalizedCookies = (0, cookieNormalizer_1.normalizeCookies)(account.credentials.cookies);
