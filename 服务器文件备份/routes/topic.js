...
const topicSelectionService_1 = require("../services/topicSelectionService");
exports.topicRouter = (0, express_1.Router)();
const topicService = new topicSelectionService_1.TopicSelectionService();
/**
 * 获取话题使用统计
 * GET /api/topics/:id/stats
 */
exports.topicRouter.get('/:id/stats', async (req, res) => {
    try {
 ...
 */
exports.topicRouter.get('/distillation/:distillationId/stats', async (req, res) => {
    try {
 ...
 */
exports.topicRouter.post('/repair-usage-count', async (req, res) => {
    try {
 ...
 * PUT /api/topics/:id
 */
exports.topicRouter.put('/:id', async (req, res) => {
    try {
        const topicId = parseInt(req.params.id);
        ...
        // 导入pool
        const { pool } = await Promise.resolve().then(() => __importStar(require('../db/database')));
        // 检查话题是否存在
        const checkResult = await pool.query('SELECT id FROM topics WHERE id = $1', [topicId]);
        if (checkResult.rows.length === 0) {
            return res.status(404).json({ error: '话题不存在' });
        }
        // 更新话题
        await pool.query('UPDATE topics SET question = $1 WHERE id = $2', [question.trim(), topicId]);
        res.json({
            success: true,
            message: '话题更新成功'
        });
 ...
 */
exports.topicRouter.delete('/:id', async (req, res) => {
    try {
        const topicId = parseInt(req.params.id);
        if (isNaN(topicId) || topicId <= 0) {
            return res.status(400).json({ error: '无效的话题ID' });
