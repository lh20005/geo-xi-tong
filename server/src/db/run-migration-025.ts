...
async function runMigration025() {
  ...
  try {
    console.log('开始执行迁移 025: 更新配额函数以支持企业图库和知识库...\n');
    
    // 读取迁移文件
    const migrationPath = path.join(__dirname, 'migrations', '025_update_quota_functions_for_gallery_knowledge.sql');
    ...
    // 测试 check_feature_quota
    const testResult = await client.query(`
      SELECT * FROM check_feature_quota(1, 'gallery_albums')
    `);
    
    console.log('测试 check_feature_quota(1, gallery_albums):');
    ...
    const testResult2 = await client.query(`
      SELECT * FROM check_feature_quota(1, 'knowledge_bases')
    `);
    
    console.log('\n测试 check_feature_quota(1, knowledge_bases):');
