...
export class ZhihuAdapter extends PlatformAdapter {
  ...
  async performLogin(page: Page, credentials: any): Promise<boolean> {
    try {
      ...
      if (credentials.cookies && credentials.cookies.length > 0) {
        ...
        // 检查是否已登录（查找头像元素）
        const isLoggedIn = await page.locator('img.AppHeader-profileAvatar').isVisible({ timeout: 5000 }).catch(() => false);
  ...
  async performPublish(page: Page, article: Article, config: PublishingConfig): Promise<boolean> {
    try {
      ...
      // 点击发布按钮
      await this.humanClick(page.getByRole('button', { name: '发布文章' }), '发布文章按钮');
  ...
  async verifyPublishSuccess(page: Page): Promise<boolean> {
    try {
      ...
      if (currentUrl.includes('success') || 
          ...
          currentUrl.includes('zhuanlan.zhihu.com')) {
  ...
  async checkLoginStatus(page: Page): Promise<boolean> {
    try {
      ...
      await this.log('warning', '❌ 知乎登录状态异常，未找到头像元素');
      return false;
    } catch (error: any) {
