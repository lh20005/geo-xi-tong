  ...
  async performLogin(
    ...
    try {
      ...
      if (credentials.cookies && credentials.cookies.length > 0) {
        ...
        await page.goto('https://mp.sohu.com/', { waitUntil: 'networkidle2' });
        ...
        console.log('[搜狐号] Cookie登录失败，尝试表单登录');
      }
  ...
  async performPublish(
    ...
    try {
      ...
      if (contentEditor) {
        ...
        console.log(`[搜狐号] 纯文字长度: ${textOnly.length} 个字符`);
        ...
        while ((match = imageRegex.exec(cleanContent)) !== null) {
          ...
          if (fs.existsSync(imagePath)) {
            ...
            console.log(`[搜狐号] 找到图片: ${path.basename(imagePath)}`);
          }
        }
        
        console.log(`[搜狐号] 共有 ${imagePaths.length} 张图片需要上传`);
        
        if (textOnly && textOnly.length > 0) {
          console.log('[搜狐号] 输入文字内容...');
          await page.keyboard.type(textOnly, { delay: 30 });
          await new Promise(resolve => setTimeout(resolve, 2000));
          console.log('[搜狐号] ✅ 文字输入完成');
        }
        
        if (imagePaths.length > 0) {
          console.log('[搜狐号] 开始上传图片...');
          
          for (let i = 0; i < imagePaths.length; i++) {
            const imagePath = imagePaths[i];
            console.log(`[搜狐号] 上传第 ${i + 1}/${imagePaths.length} 张图片: ${path.basename(imagePath)}`);
            ...
            try {
              ...
              if (uploadButton) {
                ...
                if (fileInput) {
                  ...
                  console.log(`[搜狐号] ✅ 图片 ${i + 1} 上传完成`);
                  await new Promise(resolve => setTimeout(resolve, 3000));
                }
              }
            } catch (error: any) {
              console.log(`[搜狐号] ⚠️ 图片 ${i + 1} 上传失败: ${error.message}`);
            }
          }
          
          console.log('[搜狐号] ✅ 所有图片上传完成');
        }
      }
      ...
      if (success) {
        console.log('✅ 搜狐号文章发布成功');
      }
      return success;
    } catch (error: any) {
      console.error('❌ 搜狐号文章发布失败:', error.message);
      return false;
    }
  }
