...
export class BilibiliAdapter extends PlatformAdapter {
    ...
    try {
      ...
      if (credentials.cookies && credentials.cookies.length > 0) {
        ...
        if (loginSuccess && await this.verifyCookieLogin(page)) {
          console.log('✅ 哔哩哔哩Cookie登录成功');
          return true;
        }
        
        console.log('[哔哩哔哩] Cookie登录失败，尝试表单登录');
      }
      ...
      console.log('✅ 哔哩哔哩表单登录成功');
      return true;
    } catch (error: any) {
      console.error('❌ 哔哩哔哩登录失败:', error.message);
      return false;
    }
  }
  ...
  async performPublish(
    ...
    try {
      ...
      if (contentEditor) {
        ...
        while ((match = imageRegex.exec(cleanContent)) !== null) {
          ...
          if (fs.existsSync(imagePath)) {
            ...
            console.log(`[哔哩哔哩] 找到图片: ${path.basename(imagePath)}`);
          }
        }
        
        console.log(`[哔哩哔哩] 共有 ${imagePaths.length} 张图片需要上传`);
        
        if (textOnly && textOnly.length > 0) {
          console.log('[哔哩哔哩] 输入文字内容...');
          await page.keyboard.type(textOnly, { delay: 30 });
          await new Promise(resolve => setTimeout(resolve, 2000));
          console.log('[哔哩哔哩] ✅ 文字输入完成');
        }
        
        if (imagePaths.length > 0) {
          console.log('[哔哩哔哩] 开始上传图片...');
          
          for (let i = 0; i < imagePaths.length; i++) {
            const imagePath = imagePaths[i];
            console.log(`[哔哩哔哩] 上传第 ${i + 1}/${imagePaths.length} 张图片: ${path.basename(imagePath)}`);
            ...
            try {
              ...
              if (uploadButton) {
                ...
                if (fileInput) {
                  ...
                  console.log(`[哔哩哔哩] ✅ 图片 ${i + 1} 上传完成`);
                  await new Promise(resolve => setTimeout(resolve, 3000));
                }
              }
            } catch (error: any) {
              console.log(`[哔哩哔哩] ⚠️ 图片 ${i + 1} 上传失败: ${error.message}`);
            }
          }
          
          console.log('[哔哩哔哩] ✅ 所有图片上传完成');
        }
      }
      ...
      if (success) {
        console.log('✅ 哔哩哔哩文章发布成功');
      }
      return success;
    } catch (error: any) {
      console.error('❌ 哔哩哔哩文章发布失败:', error.message);
      return false;
    }
  }
