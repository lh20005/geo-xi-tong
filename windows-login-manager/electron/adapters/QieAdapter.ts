 ...
 * 企鹅号适配器
 * 
 * 从服务器端迁移: server/src/services/adapters/QieAdapter.ts
 * 改动说明:
...
export class QieAdapter extends PlatformAdapter {
  ...
  private prepareTags(article: Article, config: PublishingConfig): string {
    if (config.tags && config.tags.length > 0) {
      return config.tags.slice(0, 9).join(' ');
    }
    
    if (article.keyword) {
      const keywords = article.keyword.split(/[,，、\s]+/).filter(k => k.trim());
      return keywords.slice(0, 9).join(' ');
    }
    
    return article.title.slice(0, 8);
  }
  ...
  async verifyPublishSuccess(page: Page): Promise<boolean> {
    try {
      ...
      if (currentUrl.includes('/content') || currentUrl.includes('/article')) {
