 ...
 * CSDN适配器
 * 
 * 从服务器端迁移: server/src/services/adapters/CSDNAdapter.ts
 * 改动说明:
 * - 修改导入语句为 ES 模块格式
...
export class CSDNAdapter extends PlatformAdapter {
  ...
  async performLogin(page: Page, credentials: any): Promise<boolean> {
    try {
      ...
      if (credentials.cookies && credentials.cookies.length > 0) {
        ...
        const isLoggedIn = await page.locator('.hasAvatar').isVisible({ timeout: 5000 }).catch(() => false);
  ...
  async checkLoginStatus(page: Page): Promise<boolean> {
    try {
      await this.log('info', '开始检查CSDN登录状态');
      ...
      if (avatarVisible) {
        await this.log('info', '✅ CSDN登录状态正常');
        return true;
      }
