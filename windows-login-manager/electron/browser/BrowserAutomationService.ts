 ...
 * 浏览器自动化服务 (Playwright)
 * 
 * 从服务器端迁移: server/src/services/BrowserAutomationService.ts
 * 改动说明:
 * - 移除 publishingService 依赖（服务器端日志服务）
 * - 使用本地 TaskService 记录日志
 * - 调整导入路径
 */

import { chromium, Browser, Page, BrowserContext } from 'playwright';
import { getStandardBrowserConfig, findChromeExecutable } from './browserConfig';
 ...
 * 日志回调函数类型
 */
export type LogCallback = (level: 'info' | 'warn' | 'error', message: string, details?: any) => void;
...
export class BrowserAutomationService {
  ...
  private defaultOptions: BrowserOptions = {
    headless: false, // Windows端默认显示浏览器
    timeout: 30000,
    viewport: {
      width: 1920,
      height: 1080
   ...
   * @param url 目标 URL
   * @param taskId 任务 ID（可选，用于日志记录，保持与服务器端接口一致）
   */
  async navigateTo(page: Page, url: string, taskId?: string): Promise<void> {
    try {
