...
class ToutiaoLoginManager {
  ...
  async login(parentWindow: BrowserWindow): Promise<ToutiaoLoginResult> {
    ...
    try {
      ...
      try {
        ...
        log.warn('[Toutiao] 后端同步失败，将降级使用本地保存:', error);
      }
  ...
  private async saveAccount(account: any): Promise<void> {
    try {
      log.info('[Toutiao] 保存账号到本地 SQLite...');
      ...
      log.info(`[Toutiao] 账号保存到本地 SQLite 成功, ID: ${result.accountId}`);
    } catch (error) {
      log.error('[Toutiao] 保存账号到本地 SQLite 失败:', error);
      throw error;
    }
  }
  ...
  private async syncToBackend(account: any): Promise<any> {
    try {
      ...
      if (result.success) {
        ...
        log.error('[Toutiao] 账号同步失败:', result.error);
