...
class JianshuLoginManager {
  ...
  async login(parentWindow: BrowserWindow): Promise<JianshuLoginResult> {
    ...
    try {
      ...
      try {
        ...
        log.warn('[Jianshu] 后端同步失败，将降级使用本地保存:', error);
      }
      
      // 10. 始终保存到本地（无论后端同步是否成功）
  ...
  private async saveAccountLocally(account: any): Promise<void> {
    try {
      log.info('[Jianshu] 保存账号到本地 SQLite...');
      ...
      log.info(`[Jianshu] 账号保存到本地 SQLite 成功, ID: ${result.accountId}`);
    } catch (error) {
      log.error('[Jianshu] 保存账号到本地 SQLite 失败:', error);
      throw error;
    }
  }
