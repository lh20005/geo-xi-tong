...
export default function ArticleListPage() {
  ...
  const loadArticles = useCallback(async () => {
    setLoading(true);
    try {
      const apiFilters: any = { publishStatus: filters.publishStatus };
      if (filters.keyword) apiFilters.keyword = filters.keyword;
      if (filters.searchKeyword && filters.searchKeyword.trim()) apiFilters.keyword = filters.searchKeyword.trim();
  ...
  useEffect(() => {
    ...
    setFilters({ publishStatus: status || 'all', keyword: keyword || '', searchKeyword: search || '' });
  ...
  useEffect(() => {
    ...
    if (newSearch !== location.search.substring(1)) navigate(`?${newSearch}`, { replace: true });
  ...
  const handleEditorClose = () => { setEditorVisible(false); setEditModal(null); };
  const handleEditorSave = () => { refreshAll(); };
  ...
  const handleClearFilters = () => {
    setFilters({ publishStatus: 'all', keyword: '', searchKeyword: '' });
    setPage(1);
  };
  ...
  const columns = [
    {
      title: (<Checkbox checked={isAllSelected} indeterminate={isSomeSelected} onChange={(e) => handleSelectAll(e.target.checked)} />),
      key: 'checkbox', width: 50, align: 'center' as const,
      render: (_: any, record: Article) => (<Checkbox checked={selectedIds.has(record.id)} onChange={(e) => handleSelectOne(record.id, e.target.checked)} />),
    },
    { title: '转化目标', dataIndex: 'conversionTargetName', key: 'conversionTargetName', width: 120, align: 'center' as const, render: (text: string) => text ? <Tag color="orange">{text}</Tag> : <span style={{ color: '#999' }}>-</span> },
    { title: '文章设置', dataIndex: 'articleSettingName', key: 'articleSettingName', width: 120, align: 'center' as const, render: (text: string) => text ? <Tag color="purple">{text}</Tag> : <span style={{ color: '#999' }}>-</span> },
    { title: '关键词', dataIndex: 'keyword', key: 'keyword', width: 120, align: 'center' as const, render: (text: string) => <Tag color="blue">{text}</Tag> },
    { title: '蒸馏结果', dataIndex: 'topicQuestion', key: 'topicQuestion', width: 200, align: 'center' as const, render: (text: string) => text ? <Tag color="green">{text}</Tag> : <span style={{ color: '#999' }}>-</span> },
    { title: '文章标题', dataIndex: 'title', key: 'title', width: 200, align: 'center' as const, ellipsis: true, render: (text: string) => text ? <span>{text}</span> : <span style={{ color: '#999' }}>-</span> },
    { title: '发布状态', dataIndex: 'isPublished', key: 'isPublished', width: 100, align: 'center' as const, render: (isPublished: boolean) => isPublished ? <Tag color="success">已发布</Tag> : <Tag color="default">草稿</Tag> },
    { title: '创建时间', dataIndex: 'createdAt', key: 'createdAt', width: 170, align: 'center' as const, render: (text: string) => new Date(text).toLocaleString('zh-CN') },
    {
  ...
  return (
    <div style={{ padding: 24 }}>
      <Row gutter={16} style={{ marginBottom: 24 }}>
        <Col span={8}><Card size="small" hoverable onClick={() => handleStatsClick('all')} style={{ textAlign: 'center' }}><Statistic title="总文章数" value={stats?.total ?? 0} valueStyle={{ color: '#1890ff' }} /></Card></Col>
        <Col span={8}><Card size="small" hoverable onClick={() => handleStatsClick('published')} style={{ textAlign: 'center' }}><Statistic title="已发布" value={stats?.published ?? 0} valueStyle={{ color: '#52c41a' }} /></Card></Col>
        <Col span={8}><Card size="small" hoverable onClick={() => handleStatsClick('unpublished')} style={{ textAlign: 'center' }}><Statistic title="未发布" value={stats?.unpublished ?? 0} valueStyle={{ color: '#faad14' }} /></Card></Col>
      </Row>

      <div style={{ marginBottom: 16, background: '#f8fafc', padding: 16, borderRadius: 8 }}>
        <Row gutter={16}>
          <Col span={5}>
            <div style={{ marginBottom: 8 }}><span style={{ color: '#64748b', fontSize: 14 }}><SearchOutlined /> 发布状态</span></div>
            <Select size="large" style={{ width: '100%' }} value={filters.publishStatus} onChange={(value) => handleFilterChange('publishStatus', value)}>
              <Option value="all">全部</Option><Option value="published">已发布</Option><Option value="unpublished">未发布</Option>
            </Select>
          </Col>
          <Col span={5}>
            <div style={{ marginBottom: 8 }}><span style={{ color: '#64748b', fontSize: 14 }}><SearchOutlined /> 关键词筛选</span></div>
          ...
          <Col span={7}>
            <div style={{ marginBottom: 8 }}><span style={{ color: '#64748b', fontSize: 14 }}><SearchOutlined /> 搜索内容</span></div>
      ...
      <Card style={{ marginBottom: 16, backgroundColor: '#e6f7ff' }}>
        <Space>
          {selectedIds.size > 0 ? (<><Text strong>已选中 {selectedIds.size} 篇文章</Text><Button danger icon={<DeleteOutlined />} onClick={handleBatchDelete}>删除选中</Button></>) : (<Text type="secondary">可以在下方表格中选择文章进行批量操作</Text>)}
        </Space>
      </Card>
