# ✅ 完整支付测试指南 - 所有问题已解决

## 🎯 问题总结与解决方案

### 问题 1: 服务器启动崩溃
**原因:** `wechatpay-axios-plugin` 初始化时调用微信支付 API 验证证书，返回 404 导致崩溃

**解决方案:** 
- 延迟初始化微信支付服务
- 只在真正需要时才初始化
- 添加错误处理，防止崩溃

### 问题 2: CORS 跨域错误
**原因:** 环境变量配置在错误的文件中（根目录 vs server 目录）

**解决方案:**
- 删除根目录的 `.env`
- 统一使用 `server/.env`
- 修改所有读取环境变量的代码路径

### 问题 3: 前端无法连接后端 API
**原因:** 通过 ngrok 访问时，前端尝试连接 `localhost:3000`

**解决方案:**
- 构建 landing 页面静态文件
- 后端提供静态文件服务
- 前端自动检测 ngrok 环境并使用正确的 API 地址

---

## ✅ 当前配置状态

### 1. 环境变量 (server/.env)
```env
# 数据库
DATABASE_URL=postgresql://lzc@localhost:5432/geo_system

# 管理员账号
ADMIN_USERNAME=lzc2005
ADMIN_PASSWORD=Woshixiaogou2005

# CORS 配置
ALLOWED_ORIGINS=http://localhost,http://localhost:5173,http://localhost:5174,http://localhost:8080,https://granolithic-pseudoprosperous-rebeca.ngrok-free.dev

# 微信支付配置
WECHAT_PAY_APP_ID=wx76c24846b57dfaa9
WECHAT_PAY_MCH_ID=1103960104
WECHAT_PAY_API_V3_KEY=3453DGDsdf3gsd564DSFDSR2N67N8Lfs
WECHAT_PAY_SERIAL_NO=305B80592042FA4A46F7A68E10044169EE13093D
WECHAT_PAY_PRIVATE_KEY_PATH=/Users/lzc/.wechat-pay/apiclient_key.pem
WECHAT_PAY_NOTIFY_URL=https://granolithic-pseudoprosperous-rebeca.ngrok-free.dev/api/payment/wechat/notify
```

### 2. 微信支付配置（公钥模式）
- 商户私钥: `/Users/lzc/.wechat-pay/apiclient_key.pem`
- 证书序列号: `305B80592042FA4A46F7A68E10044169EE13093D`
- **注意：** 使用公钥模式，不需要下载平台证书。详见官方文档：https://pay.weixin.qq.com/doc/v3/partner/4012925323

### 3. 服务状态
- ✅ 后端服务运行中 (端口 3000)
- ✅ Landing 静态文件已构建
- ✅ ngrok 隧道运行中
- ✅ CORS 配置正确
- ✅ 微信支付已配置

---

## 🚀 完整测试流程

### 步骤 1: 确认所有服务运行

```bash
# 检查后端服务
curl http://localhost:3000/api/health

# 检查 ngrok
ps aux | grep ngrok

# 查看 ngrok 控制台
open http://localhost:4040
```

### 步骤 2: 访问 ngrok 地址

```
https://granolithic-pseudoprosperous-rebeca.ngrok-free.dev
```

**首次访问提示:**
- ngrok 免费版会显示警告页面
- 点击 **"Visit Site"** 按钮继续

### 步骤 3: 登录系统

**登录信息:**
- 用户名: `lzc2005`
- 密码: `Woshixiaogou2005`

### 步骤 4: 测试支付流程

1. **选择套餐** - 点击任意套餐的"立即购买"按钮
2. **查看二维码** - 支付弹窗应该显示二维码
3. **微信扫码** - 使用微信扫描二维码
4. **完成支付** - 在微信中完成支付
5. **验证回调** - 页面应该自动检测到支付成功并跳转

---

## 🔍 验证测试结果

### 前端验证
打开浏览器控制台 (F12)，应该看到：

```
[Landing Config] 环境: {
  hostname: "granolithic-pseudoprosperous-rebeca.ngrok-free.dev",
  isNgrok: true,
  apiUrl: "https://granolithic-pseudoprosperous-rebeca.ngrok-free.dev/api"
}
```

### 后端验证
查看后端日志，应该看到：

```
✅ 提供 Landing 页面静态文件服务
🚀 服务器运行在 http://localhost:3000
```

支付成功后应该看到：

```
[微信支付] 收到支付回调
✅ 订单状态更新成功: paid
```

### 数据库验证

```bash
psql -U lzc -d geo_system -c "SELECT order_no, status, amount, plan_id FROM orders ORDER BY created_at DESC LIMIT 5;"
```

应该看到订单状态为 `paid`

---

## 🐛 常见问题排查

### Q1: 访问 ngrok 地址显示空白页

**检查:**
```bash
# 1. 确认 landing 已构建
ls -la landing/dist/

# 2. 重新构建
cd landing && npm run build

# 3. 重启后端
# 停止并重新启动 server
```

### Q2: 前端显示 "ERR_CONNECTION_REFUSED"

**原因:** 前端配置错误，仍在尝试连接 localhost

**解决:**
1. 清除浏览器缓存
2. 打开控制台查看 `[Landing Config]` 日志
3. 确认 `apiUrl` 是否正确指向 ngrok 域名

### Q3: 支付后页面没有跳转

**检查:**
1. 后端日志是否收到支付回调
2. 浏览器控制台是否有错误
3. 订单状态是否更新为 `paid`

```bash
# 查看订单状态
psql -U lzc -d geo_system -c "SELECT order_no, status FROM orders WHERE order_no = 'ORD订单号';"
```

### Q4: 创建订单失败

**可能原因:**
1. 微信支付配置错误
2. 证书文件不存在
3. 公钥 ID 不正确

**检查:**
```bash
# 1. 检查证书文件
ls -la ~/.wechat-pay/

# 2. 查看后端日志
# 应该看到微信支付初始化成功的消息
```

---

## 📝 技术实现细节

### 1. Landing 页面环境检测

```typescript
// landing/src/config/env.ts
const detectEnvironment = () => {
  const hostname = window.location.hostname;
  const isNgrok = hostname.includes('ngrok');
  
  return { isNgrok, ... };
};

// ngrok 环境配置
ngrok: {
  apiUrl: `${window.location.protocol}//${window.location.host}/api`,
  environment: 'ngrok'
}
```

### 2. 后端静态文件服务

```typescript
// server/src/index.ts
const landingDistPath = path.join(__dirname, '../../landing/dist');
if (fs.existsSync(landingDistPath)) {
  app.use(express.static(landingDistPath));
  
  // SPA 路由支持
  app.get('*', (req, res) => {
    if (!req.path.startsWith('/api')) {
      res.sendFile(path.join(landingDistPath, 'index.html'));
    }
  });
}
```

### 3. 微信支付延迟初始化

```typescript
// server/src/services/PaymentService.ts
private ensureInitialized() {
  if (!this.initialized) {
    try {
      this.initializeWeChatPay();
      this.initialized = true;
    } catch (error) {
      this.initializationError = error;
      throw new Error('微信支付服务初始化失败');
    }
  }
}
```

---

## 🎉 测试检查清单

- [ ] 后端服务启动成功
- [ ] ngrok 隧道运行中
- [ ] Landing 页面已构建
- [ ] 可以访问 ngrok 地址
- [ ] 可以成功登录
- [ ] 可以看到套餐列表
- [ ] 点击购买显示二维码
- [ ] 微信扫码可以打开支付页面
- [ ] 支付成功后页面自动跳转
- [ ] 订单状态更新为 paid
- [ ] 后端收到支付回调

---

## 🔄 更新 ngrok 地址

当 ngrok 重启后域名改变时：

### 1. 更新环境变量

编辑 `server/.env`:
```env
ALLOWED_ORIGINS=http://localhost,http://localhost:5173,http://localhost:5174,http://localhost:8080,https://新的ngrok域名.ngrok-free.dev
WECHAT_PAY_NOTIFY_URL=https://新的ngrok域名.ngrok-free.dev/api/payment/wechat/notify
```

### 2. 重启后端服务

```bash
# 停止现有服务
pkill -f "tsx.*src/index.ts"

# 重新启动
cd server
npm run dev
```

### 3. 重新构建 Landing (可选)

如果前端配置有缓存问题：
```bash
cd landing
npm run build
```

---

## 📞 需要帮助？

如果遇到问题：

1. **查看后端日志** - 检查服务器输出
2. **查看浏览器控制台** - 检查前端错误
3. **查看 ngrok 控制台** - http://localhost:4040
4. **检查数据库** - 查看订单状态

**快速诊断命令:**
```bash
# 检查服务状态
curl http://localhost:3000/api/health

# 检查 ngrok
curl -I https://granolithic-pseudoprosperous-rebeca.ngrok-free.dev/

# 查看最新订单
psql -U lzc -d geo_system -c "SELECT * FROM orders ORDER BY created_at DESC LIMIT 1;"
```

---

## 🎯 总结

✅ 所有配置问题已解决  
✅ 服务器稳定运行  
✅ 前端可以正确连接后端  
✅ CORS 配置正确  
✅ 微信支付已配置  
✅ 可以通过 ngrok 完整测试支付流程  

**现在可以开始完整的支付测试了！** 🚀
